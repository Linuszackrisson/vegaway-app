...
const EditOrder: React.FC<EditOrderProps> = ({ onClose }) => {
	...
	const uniqueOrderItems = Array.from(new Map(order?.items.map(item => [item.menuId, item])).values());
		...
		<div className="edit-order">
			{errorMessage !== '' && (
				<div className="edit-order__error-message">
					<button className="edit-order__error-message__close" onClick={onClose}>
						X
					</button>
					{errorMessage}
				</div>
			...
			<div className="edit-order__content" ref={contentRef}>
				<button className="edit-order__update-button" onClick={handleUpdateOrder}>
					Update
				</button>
