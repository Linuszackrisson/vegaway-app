...
const CartProductCard: React.FC<CartProductCardProps> = ({ item, editOrder, isStaffOrderDetails, onUpdateItem }) => {
	...
	const removeFromOrder = useCurrentOrderStore(state => state.removeFromOrder);
	const decreaseQuantityInOrder = useCurrentOrderStore(state => state.decreaseQuantity);
	const increaseQuantityInOrder = useCurrentOrderStore(state => state.increaseQuantity);
	...
	const itemCount = editOrder
		? currentOrderItems.filter(orderItem => orderItem.menuId === item.menuId).length
		: isStaffOrderDetails
...
return (
	<div className="card cart-product-card">
		<div className="cart-product-card__image-container">
			<img className="cart-product-card__image" src={item.imageUrl} alt={item.name} />
		</div>
		<div className="cart-product-card__info">
			<div className="cart-product-card__header">
				...
				<div className="cart-product-card__quantity-controls">
					<button className="button button--icon cart-product-card__remove-button" onClick={handleDecrease}>
						<Icon name="Minus" className="icon" />
					</button>
					<span className="cart-product-card__quantity">{itemCount}</span>
					<button className="button button--icon cart-product-card__add-button" onClick={handleIncrease}>
						<Icon name="Plus" className="icon" />
					</button>
				</div>
			</div>
			<p className="cart-product-card__price">${(item.price * itemCount).toFixed(2)}</p>
			<button className="button button--third cart-product-card__info-button" onClick={handleInfoClick}>
